Feature: Selectors
  ...
  Scenario: Replay Events after a known Event
    ...
    When Authenticated Client replays all 'Tennis Match' events, key '2023-01-09' after remembered selector position
  ...
  Scenario: Replay limited number of events after a known Event
    ...
    And remembers selector position
    When Authenticated Client replays, after remembered selector position, 5 'Ball Served, Ball Returned, Ball Out' events from 'Tennis Match', keys '2023-01-07, 2023-01-09'
    Then Event count is 5
  ...
  Scenario: Select subset of events with meta filter
    Given Authenticated Client replays 'Ball Served' events for 'Tennis Match', keys '2023-01-07'
    And remembers selector position
    Given Authenticated Client filters, after remembered selector position, events with meta filter '$.command ? (@ % 2 == 0)'
    Then Event count is 3
    And last Event is 'Ball Returned'
  ...
  Scenario: Select subset of events with meta filter after a known event
    Given Authenticated Client replays 'Ball Served' events for 'Tennis Match', keys '2023-01-07'
    And remembers selector position
    Given Authenticated Client filters, after remembered selector position, 2 events with meta filter '$.command ? (@ % 2 == 0)'
    Then Event count is 2
